...
enum YTDLPService {
    private static let requestRetryCount = 2
    private static let requestRetryDelayNanoseconds: UInt64 = 1_000_000_000
    ...
    static func fetchVideoMetadata(url: String) async throws -> VideoMetadata {
        let output = try await runChecked(command: .metadata(url: url))
    ...
    static func fetchTitle(url: String) async throws -> String {
        let output = try await runChecked(command: .title(url: url))
    ...
    private static func resolveYouTubeCollection(
        ...
        let output = try await runChecked(
            command: .collection(
                ...
                dateAfter: dateRange == .allTime ? nil : dateRange.ytdlpDateString
            )
        )
        ...
        let entries = flattened.compactMap { entry -> ResolvedRemoteQueueItem? in
            ...
            return ResolvedRemoteQueueItem(
    ...
    private static func downloadYouTubeAudio(
        ...
        let output = try await runStreaming(
            command: .downloadAudio(url: url, outputTemplate: outputTemplate),
    ...
    private static func runChecked(command: YTDLPInvocation.Command) async throws -> String {
        let request = YTDLPInvocation.makeRequest(for: command)

        for attempt in 0..<requestRetryCount {
            do {
                return try await SubprocessRunner.runChecked(
                ...
                guard shouldRetryRequest(error), attempt < requestRetryCount - 1 else {
                    throw error
                }
                AppLogger.info(
                    "YTDLP",
                    "Retrying \(command.logLabel) after transient failure: \(error.localizedDescription)"
                )
                try? await Task.sleep(nanoseconds: requestRetryDelayNanoseconds)
            }
        }

        throw YTDLPError.downloadFailed("Unexpected yt-dlp retry state")
    }

    private static func runStreaming(
        command: YTDLPInvocation.Command,
        onOutputLine: @escaping @Sendable (String) -> Void
    ) async throws -> SubprocessRunner.Output {
        let request = YTDLPInvocation.makeRequest(for: command)
        return try await SubprocessRunner.runStreaming(
            executable: StoragePaths.ytdlpBinary,
            arguments: request.arguments,
            environment: request.environment,
    ...
    private static func shouldRetryRequest(_ error: Error) -> Bool {
        switch error {
        case SubprocessError.timedOut:
            return true
        case SubprocessError.executionFailed(let output, let code):
            ...
            return code == 1
                ...
                || message.contains("unable to download")
        default:
            return false
        }
    }
