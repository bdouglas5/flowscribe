...
final class QueueManager {
    ...
    func suspend() {
        ...
        isSuspended = true
        AppLogger.info("Queue", "Queue suspended")
    }

    func resume() {
        guard isSuspended else { return }
        isSuspended = false
        AppLogger.info("Queue", "Queue resumed")
        startProcessingIfNeeded()
    }
    ...
    private func processQueue() async {
        while let next = await nextWaitingItem() {
            do {
                ...
                let presentation = QueueErrorPresentation.make(
                    from: error,
                    remoteSource: next.remoteSource
                )
                await MainActor.run {
                    next.status = .failed
                    next.errorMessage = presentation.technicalMessage
                    next.userFacingError = presentation.userMessage
                    next.recoveryAction = presentation.recoveryAction
                }
    ...
    private func nextWaitingItem() -> QueueItem? {
        ...
        return items.first { $0.status == .waiting }
    }
}
