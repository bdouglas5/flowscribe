...
final class DatabaseManager {
    ...
    private func migrate() throws {
        ...
        migrator.registerMigration("v6_add_transcript_chat_messages") { db in
            try db.create(table: "transcriptChatMessage") { t in
                t.autoIncrementedPrimaryKey("id")
                t.column("transcriptId", .integer)
                    .notNull()
                    .indexed()
        ...
        migrator.registerMigration("v7_add_transcript_markdown_files") { db in
            try db.create(table: "transcriptMarkdownFile") { t in
                t.autoIncrementedPrimaryKey("id")
                t.column("transcriptId", .integer)
                    .notNull()
                    .indexed()
                ...
                t.column("sourcePrompt", .text)
                t.column("legacyAIResultId", .integer)
                    .references("transcriptAIResult", onDelete: .setNull)
                t.column("createdAt", .datetime).notNull()
                t.column("updatedAt", .datetime).notNull()
            }

            try db.create(
                index: "idx_transcriptMarkdownFile_legacyAIResultId",
                on: "transcriptMarkdownFile",
                columns: ["legacyAIResultId"]
            )
        }

        migrator.registerMigration("v8_add_transcript_context_cache") { db in
            try db.create(table: "transcriptContextCache") { t in
                t.autoIncrementedPrimaryKey("id")
                t.column("transcriptId", .integer)
                    .notNull()
                ...
                t.column("memorySummary", .text).notNull().defaults(to: "")
                t.column("errorMessage", .text)
                t.column("createdAt", .datetime).notNull()
                t.column("updatedAt", .datetime).notNull()
            }

            try db.create(table: "transcriptContextChunk") { t in
                t.autoIncrementedPrimaryKey("id")
                t.column("cacheId", .integer)
                    .notNull()
                    .indexed()
                    .references("transcriptContextCache", onDelete: .cascade)
                t.column("transcriptId", .integer)
                    .notNull()
                    .indexed()
                    .references("transcript", onDelete: .cascade)
            ...
            try db.create(
                index: "idx_transcriptContextChunk_cache_sort",
                on: "transcriptContextChunk",
                columns: ["cacheId", "sortOrder"],
                unique: true
            )
        }
